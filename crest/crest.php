...
class CRest
{
    ...
    public static function installApp()
    {
        ...
        if (($_REQUEST['event'] ?? null) == 'ONAPPINSTALL' && !empty($_REQUEST['auth'])) {
        ...
        } elseif (($_REQUEST['PLACEMENT'] ?? null) == 'DEFAULT') {
            ...
            $result['install'] = static::setAppSettings(
                [
                    ...
                    'client_endpoint' => 'http://' . htmlspecialchars($_REQUEST['DOMAIN']) . '/rest/',
                ],
                true
            );
        }
    ...
    protected static function callCurl($arParams)
    {
        ...
        if ($arSettings !== false) {
            if (isset($arParams['this_auth']) && $arParams['this_auth'] == 'Y') {
                $url = defined("C_REST_OAUTH_URL")
                    ? C_REST_OAUTH_URL . 'oauth/token/'
